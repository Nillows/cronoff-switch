document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateAliases() {
        ...
        // Build the 'off' alias
        let offAlias = "alias " + aliasName + "off='crontab -l | awk '\\''/^#/{print;next}/^" + escapedCronJobForAwk + "$/{print \"#\" $0;next}1'\\'' > /tmp/crontab.txt && if grep -q \"^#" + escapedCronJobForGrep + "$\" /tmp/crontab.txt; then crontab /tmp/crontab.txt; printf \"" + capitalizedAliasName + " has been disabled.\\n\"; else printf \"Crontab task not found!\\n\"; fi && rm /tmp/crontab.txt'";

        // Build the 'on' alias
        let onAlias = "alias " + aliasName + "on='crontab -l | awk '\\''/^#" + escapedCronJobForAwk + "$/{sub(/^#/,\"\",$0);print;next}1'\\'' > /tmp/crontab.txt && if grep -q \"^" + escapedCronJobForGrep + "$\" /tmp/crontab.txt; then crontab /tmp/crontab.txt; printf \"" + capitalizedAliasName + " has been enabled.\\n\"; else printf \"Crontab task not found!\\n\"; fi && rm /tmp/crontab.txt'";
